...
export function WinnerScreen({ theme, feed, crew, groupScore, gameOver = false, onClose }) {
  ...
  const has = s.total > 0;
  // The leader headline always crowns the king — "ALL HAIL / THE KEBAB KING"
  // (or "...KINGS" for a tie) — on two fixed lines, auto-sized to fill the width
  // (longest line decides the size, both lines share it, no extra wrapping). The
  // surrounding labels still switch between the live standings and the final result.
